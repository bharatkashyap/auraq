<template>
  <div
    class="post fade-in flex-column"
    :class="{ headline: post.isHeadline }"
    :id="post.id"
  >
    <div class="fadeIn d-flex mb-5 flex-column align-items-center">
      ...
      >
        ...
        <div
          id="post-heading"
          class="mt-2 align-items-start text-center display-4"
        >
          {{ post.Heading }}
        </div>
        <div
        ...
        <div id="post-date" class="mt-1 ml-3 text-center font-size-sm">
...
export default {
  ...
  computed: {
    readingTime: function() {
      if (this.post.Notes)
        return Math.round(
          (this.post.Notes.split(" ").length / this.averageReadingSpeed) * 0.6
        );
      else return 0;
    },
    authors: function() {
