    ...
    <div class="post-post d-flex flex-column align-items-center mt-3">
        <div class="d-flex flex-row">
            <div id="likes" class="align-self-center mx-3">
                <img class="icon-svg icon-svg-small" :class="{liked: mutableLiked}" src="@/assets/img/like.svg" @click="signIn(toggleLike)" />
                <span class="text-muted number ml-1" id="likes-number">{{count(likes)}}</span>
            </div>
...
export default {
    ...
    methods: {
        ...
        typingStatus(status) {
            if(this.comment.length) { this.typing = true; this.commentsListVisibility = true;}
            else { this.typing = status; this.commentsListVisibility = status; }
        },
        toggleViewComments() {
