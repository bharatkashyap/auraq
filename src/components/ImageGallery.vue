<template>
    <div>
        <div tabindex="-1" v-if="!isStub" id="post-image-gallery" ref="imageGallery" v-observe-visibility="visibilityObserver" @scroll.passive="handleGalleryScroll">
            <span id="gallery-count">{{selectedImageIndex+1}} of {{pics.length}}</span>
            <div id="post-image" class="flex-equal mt-1" v-for="(pic, index) in pics" :key="index" :ref="pic.id">
...
export default {
    ...
    methods: {
        ...
        handleGalleryScroll($event) {
            this.selectedImageIndex = parseInt( ($event.target.scrollLeft / this.viewport.width) + 0.1);
        },
